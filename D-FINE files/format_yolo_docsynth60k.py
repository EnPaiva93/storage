...
parquet_list = os.listdir("./docsynth60k")
parquet_list = [f for f in parquet_list if "parquet" in f]

save_path = "./layout_data_yolo/docsynth60k"
if not os.path.exists(save_path):
    os.makedirs(save_path)
    os.makedirs(os.path.join(save_path, "images"))
    os.makedirs(os.path.join(save_path, "labels"))
train_txt = open(os.path.join(save_path, "train60k.txt"), "w")
    
for parquet in tqdm.tqdm(parquet_list):
    df = pd.read_parquet(os.path.join("docsynth60k", parquet))
...
train_txt.close()
# image_list = list(open(os.path.join(save_path, "train60k.txt"), "r").readlines())[:1000]
